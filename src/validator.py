...
class Validator:
    ...
    def eval(self, checkpoint_name=None):
        ...
        loop_bar = tqdm(self.valid_set, desc='EVALUATING', colour='blue')
        self.logger.new_epoch()
        
        # Loop the validation set
        for batch_image, batch_gt_depth, batch_velo_depth in loop_bar:

            B = batch_image.shape[0] # Batch size

            batch_image = batch_image.to('cuda') # shape = (B, 3, H_image, W_image)

            # batch predict
            self.model.eval()
            with torch.no_grad():
                batch_pred, conf_scores = self.model(batch_image) # shape = (B, 1, H_pred, W_pred)
